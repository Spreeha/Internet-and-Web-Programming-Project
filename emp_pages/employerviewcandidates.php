...
<br>
    ...
    <div class="content">
<center><h1> CANDIDATE PROFILE </h1></center>
<hr>
</div>
        <div class="info">
          <br>
...
//$sql1 = "SELECT cname,title FROM job WHERE empuser='$var' ORDER BY date DESC ";  
  ...
  $results = mysqli_query($con,"SELECT name,qualification,experience,skills FROM candidateprofile");
  
  while($rows1 = mysqli_fetch_assoc($results))
{
  $name=$rows1['name']; //echo $id; echo "hello";echo $row1['title'];
  $qualification=$rows1['qualification'];
  $experience=$rows1['experience'];
  $skills=$rows1['skills'];
 // echo $titl." ".$comp." ".$can;
...
<h1> Name:<a href="#"><?php echo $name; ?></a> </h1>
<hr>
        <div class="info">
<?php // echo $comp." ".$can; ?>
<center><p> Qualification: <?php echo $qualification; ?> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Years of Experience : <?php echo $experience; ?>  &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Skills: <?php echo $skills; ?><br><br> <!--<a href="javascript:void(0)?;" onclick="theVar=confirm('Shortlist this candidate. Are you sure?');setTimeout('if(theVar){window.location=\'employershortcandidates.php?nam=<?php echo $can;?>\'}', 0);"> <input type = "submit" name="short" value="SHORTLIST THIS CANDIDATE" class="button"></a>
