...
<br>
    ...
    <div class="content">
<center><h1> My Shortlisted Jobs </h1></center>
<hr>
</div>
        <div class="info">
          <br>
...
/*while($rows = mysqli_fetch_assoc($result1))
{
 ...
 // $sqls = "SELECT canuser,resumeid,title,Status FROM applyresumetojobs WHERE title='$titl'";  
  $results = mysqli_query($con,"SELECT canuser,resumeid,title,Status FROM applyresumetojobs WHERE canuser='$var' and Status=3");
...
<h1> Title:<a href="#"><?php echo $rows1['title']; ?></a> </h1>
<hr>
        <div class="info">
<?php // echo $comp." ".$can; ?>
<center><p> ResumeID: <?php echo $id; ?> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
<?php if($row1['Status']=3)?>Status: Shortlisted   <!--Applying Date :--> <?php echo $date; ?>  &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Candidate Name: <?php echo $can; ?><br><br> <!--<a href="javascript:void(0)?;" onclick="theVar=confirm('Shortlist this candidate. Are you sure?');setTimeout('if(theVar){window.location=\'employershortcandidates.php?nam=<?php echo $can;?>\'}', 0);"> <input type = "submit" name="short" value="SHORTLIST THIS CANDIDATE" class="button"></a>
